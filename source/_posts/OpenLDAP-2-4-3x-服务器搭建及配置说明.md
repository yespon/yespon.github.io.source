...
其实只要在后面编译openldap时能找到lib和include下的库就行了，不止上面设置环境变量一种办法，解决办法还有直接复制对应的库文件到/usr/lib和/usr/include，或修改/etc/ld.so.conf.d，选其一即可。
编译安装：
...
![image](OpenLDAP-2-4-3x-服务器搭建及配置说明/p_config_dit.png)
使用这种新的配置目录的好处在于：
